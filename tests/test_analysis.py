...
def test_get_or_make_file(empty_analysis, analysis):
    ...
    empty_analysis.get_or_make_file(filekey,
                                    filename,
                                    task)
...
def test_get_files(analysis):
    ...
    assert len(files) == 9
    assert all(['ta' in file.filekeys
                for file
                in files])
...
def test_get_tasks(analysis):
    ...
    assert len(tasks) == 2
    assert all([task.task_name == 'bam2ta'
                for task in tasks])
    assert len(tasks2) == 1
...
def test_search_up(analysis):
    ...
    files2 = analysis.search_up(task2,
                                'trim_adapter',
                                'fastqs',
                                inputs=True)
    assert len(files2) == 2
    task3 = analysis.get_tasks('ataqc')[0]
    files3 = analysis.search_up(task3,
                                'trim_adapter',
                                'fastqs',
                                inputs=True)
