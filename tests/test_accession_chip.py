...
@pytest.fixture
def mock_accession_patched_qc(mocker, mock_accession, gsfile):
    """
    Performs shared patches to qc metric maker functions. Dependents on this fixture
    should patch mock_accession.backend.read_json() to supply the appropriate qc stub
    """
    mocker.patch.object(mock_accession, "file_has_qc", return_value=False)
    mocker.patch.object(mock_accession.analysis, "get_files", return_value=[gsfile])
    mocker.patch.object(mock_accession, "get_chip_pipeline_replicate", return_value=REP)
    mocker.patch.object(mock_accession, "queue_qc", lambda output, *args: output)
...
histone_replication_qc = {
    ...
    "replication": {
        "reproducibility": {
            "overlap": {
                **tf_replication_qc["replication"]["reproducibility"][
                    "idr"
                ]  # type: ignore
            }
        }
...
def test_get_chip_pipeline_replication_method(pipeline_type, expected):
    method = Accession.get_chip_pipeline_replication_method(
...
def test_maybe_preferred_default(mocker, mock_accession, gsfile, qc, expected):
    mocker.patch.object(mock_accession.analysis, "get_files", return_value=[gsfile])
    mocker.patch.object(mock_accession.analysis.backend, "read_file", return_value=qc)
    result = mock_accession.maybe_preferred_default(gsfile)
...
def test_add_chip_mapped_read_length(
    mocker, mock_accession, gsfile, log_contents, expectation, expected_value
):
    mocker.patch.object(mock_accession.analysis, "search_up", return_value=[gsfile])
    mocker.patch.object(
        mock_accession.analysis.backend, "read_file", return_value=log_contents
    )
    with expectation:
        result = mock_accession.add_mapped_read_length(gsfile)
...
def test_get_chip_pipeline_replicate(mocker, mock_accession, gsfile):
    mocker.patch.object(mock_accession.analysis, "search_up", return_value=[gsfile])
    mocker.patch.object(
    ...
    rep = mock_accession.get_chip_pipeline_replicate(gsfile)
    assert rep == "rep1"


def test_get_chip_pipeline_replicate_raises(mocker, mock_accession, gsfile):
    mocker.patch.object(mock_accession.analysis, "search_up", return_value=[gsfile])
    mocker.patch.object(
        mock_accession.analysis,
        "metadata",
    ...
    with pytest.raises(ValueError):
        mock_accession.get_chip_pipeline_replicate(gsfile)
