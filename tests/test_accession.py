...
def test_get_or_make_step_run(accession):
    bowtie_step = accession.steps_and_params_json[0]
    step_run = accession.get_or_make_step_run(accession.lab_pi,
...
def test_make_file_obj(accession):
    ...
    obj = accession.make_file_obj(bam,
...
def test_accession_steps(accession):
    ...
    for file in accession.new_files:
        if (file.get('output_type') == 'optimal idr thresholded peaks' and
                file.get('file_format') == 'bed' and
                file.get('file_format_type') == 'narrowPeak'):
